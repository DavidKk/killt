...
~function () {
  ...
  Engine.extend(function (engine) {
    var HELPER_SYNTAX = '(=|-|!|#|!#)?\\s*([^|]+?(?:\\s*(?:\\|\\||\\&\\&)\\s*[^|]+?)*)\\s*\\|\\s*([^:\\|]+?)\\s*(?:\\:\\s*([^\\|]+?))?\\s*(\\|\\s*[\\w\\W]+?)?';
    // const HELPER_REGEXP       = engine._compileRegexp(HELPER_SYNTAX)
    var HELPER_INNER_SYNTAX = '\\s*([\\w\\W]+?\\s*\\\([\\w\\W]+?\\\))\\s*\\|\\s*([^:]+?)\\s*(:\\s*([^\\|]+?))?$';
    var HELPER_INNER_REGEXP = engine._compileRegexp(HELPER_INNER_SYNTAX);
     ...
     */
    .$registerSyntax('sourceOpen', 'source(.+?)', 'source$1').$registerSyntax('sourceClose', '\\/source', '/source').$registerSyntax('include', 'include\\s*([\\w\\W]+?)\\s*(,\\s*([\\w\\W]+?))?\\s*', function ($all, $1, $2, $3) {
  ...
  var Server = function (_ref7) {
    _inherits(Server, _ref7);
    ...
    function Server(options) {
      ...
      var _this9 = _possibleConstructorReturn(this, (Server.__proto__ || Object.getPrototypeOf(Server)).call(this, options));
    ...
    _createClass(Server, [{
      ...
      value: function compileSource(source, options) {
        return _get(Server.prototype.__proto__ || Object.getPrototypeOf(Server.prototype), 'compile', this).call(this, source, options);
      }
      ...
      value: function renderSource(source, data, options) {
        return _get(Server.prototype.__proto__ || Object.getPrototypeOf(Server.prototype), 'render', this).call(this, source, data, options);
      }
      ...
      value: function compile(template, callback) {
        ...
        this.readFile(template).then(function (source) {
          /**
            ...
            (function () {
              ...
              forEach(dependencies, function (dependency) {
                ...
                if (!_this11._cache(dependency)) {
                  var promise = new Promise(function (resolve, reject) {
                    _this11.compile(dependency, function (err, render) {
              ...
              if (0 < promises.length) {
                Promise.all(promises).then(function () {
                  var render = _this11.$compile(origin);
                  _this11._cache(template, render);
                ...
                var _render = _this11.$compile(origin);
                _this11._cache(template, _render);
          ...
          // 找不到任何子模板直接编译模板
              ...
              var _render2 = _this11.$compile(origin);
              _this11._cache(template, _render2);
      ...
      key: 'render',
      value: function render(template, data, callback, options) {
        if (is('Function')(data)) {
      ...
      value: function compileSync(template) {
         ...
         */
        var source = this.readFileSync(template, options);
        ...
        if (0 < dependencies.length) {
          forEach(dependencies, function (dependency) {
            // check if dependency is already exists
            // 检测子模板是否已经存在
            if (!_this12._cache(dependency)) {
              _this12.compileSync(dependency, options);
            }
          });
        }
